...
biogrid <- read_tsv(
  "/mnt/picea/projects/seidr/gold_standard/dmel_biogrid_all.tsv",
  skip = 1,
...
sc_sf <- SeidrFileFromPath("/mnt/picea/projects/seidr/networks/dmel/results/aggregate/irp.sf")
...
comps <- list(c("Not Connected", "Genetic"),
              c("Not Connected", "Physical"))
algos <- unique(sc_edges_long$Algorithm)
algos <- algos[algos!="ARACNE"]
c(split(c(paste(algos, "Not Connected"), paste(algos, "Genetic")),
        c(1:13, 1:13)),
  split(c(paste(algos, "Not Connected"), paste(algos, "Physical")),
        c(1:13, 1:13))) -> comps
sc_edges_long %>%
  mutate(Group = paste(Algorithm, Evidence)) %>%
  ungroup() -> sc_edges_long
...
ggsave("~/seidr-plots/dm_biogrid_edgerank.pdf", width = 16, height = 9)
